...
>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-emerald-400">Sampah Roles</h1>
            <p class="text-gray-400 text-sm mt-1">
                Data role yang telah dihapus (soft delete)
            </p>
        </div>
                ...
                @forelse($roles as $role)
                    <tr class="border-t border-white/10 hover:bg-white/5 transition">
                        ...
                        <td class="px-6 py-4 text-right space-x-2">
                            ...
                            {{-- Jangan tampilkan aksi jika role default --}}
                            @if(!$role->isDefault())

                                {{-- Restore --}}
                                <button
                                    @click="openModal(
                                        '{{ route('super_admin.roles.restore', $role->id) }}',
                                        'POST',
                                        'Yakin ingin merestore role ini?'
                                    )"
                                ...
                                <button
                                    @click="openModal(
                                        '{{ route('super_admin.roles.forceDelete', $role->id) }}',
                                        'DELETE',
